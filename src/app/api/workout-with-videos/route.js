...
/**
 * POST /api/workout-with-videos
 * This route generates workout plans and enhances them with YouTube video links
...
export async function POST(req) {
    try {
        ...
        const workoutPlan = await getWorkoutPlanFromGemini(formData);
        ...
        console.log("Found videos for exercises:", exerciseWithVideos.map(e => e.name));
...
/**
 * Gets a workout plan from Gemini based on form data
 */
async function getWorkoutPlanFromGemini(formData) {
    ...
    Format your response in Markdown with headings, bullet points, line breaks, etc.
    `;
...
function enhanceWorkoutPlanWithVideos(workoutPlan, exerciseWithVideos) {
    ...
    console.log("Enhancing plan with videos for exercises:", exerciseWithVideos.map(e => e.name));
    ...
    for (const exercise of exerciseWithVideos) {
        if (exercise.url) {
            ...
            if (headingRegex.test(enhancedPlan)) {
                ...
                console.log(`Could not find exact heading match for: "${exercise.name}"`);
                ...
                if (flexibleHeadingRegex.test(enhancedPlan)) {
                    ...
                    console.log(`Could not find flexible match for: "${exercise.name}"`);
                }
            }
        }
    }
